...
public static class Networking
{
    ...
    public static void UpdateItem(int levelID, int levelVersion, int newMinTurns, InputHistory inputHistory, Action<string> succes, Action<string> failure)
    {
        var method = NetworkingMethods.Put<LevelsDatabaseStructure>($"{NetworkingConstants.ROOT_URL}{NetworkingConstants.UPDATE_ITEM_ROUTE}",
            JsonUtility.ToJson(new ModifyLevelRequest(levelID, levelVersion, newMinTurns, inputHistory)), succes, failure,
            null);
        Routiner.StartCouroutine(method);
    }
}